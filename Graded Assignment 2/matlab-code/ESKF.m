classdef ESKF
    ...
    methods
        ...
        function xnompred = predictNominal(obj, xnom, acc, omega, Ts)
            ...
            gyroBiasPred = gyroBias - Ts * obj.pGyro * eye(3) * gyroBias;
        ...
        function [xinjected, Pinjected] = inject(~, xnom, deltaX, P)
            ...
            % TODO: neatify
            % pure quat, or [1; deltaTheta / 2]? 
            % make sure quaterion is normalized
            xinjected = composition(xnom, deltaX); % Function normalizes quaternion
            
            % compensate for injection in the covariance
            Ginject = blkdiag(eye(6), eye(3) - crossProdMat(deltaX(7:9)/2), eye(6));
        ...
        function deltaX = deltaX(~, xnom, xtrue)
            ...
            %
            % deltaX (15 x 1): the errors such that deltaX = xtrue "-" xnom
           ...
           deltaTheta = deltaQuat(2:4) * 2; % the error state
